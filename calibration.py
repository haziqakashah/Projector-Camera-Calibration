...
def main():
    ...
    dirnames = sorted(glob.glob('./captured_*'))
    if len(dirnames) == 0:
...
def calibrate(dirnames, gc_fname_lists, proj_shape, chess_shape, chess_block_size, gc_step, black_thr, white_thr, camP, camD):
    ...
    for dname, gc_filenames in zip(dirnames, gc_fname_lists):
        ...
        image_with_corners = cv2.drawChessboardCorners(white_img, chess_shape, cam_corners, True)

        # Display the image with corners
        cv2.imwrite('test_image_with_corner.jpg', image_with_corners)
        if not res:
